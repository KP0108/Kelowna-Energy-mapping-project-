...
which(BCAA_Building_Info$CivicAddre %in% Combined_data$PRINT_ADDRESS)
...
d <- unique(Address_points_Pi_NH$OBJECTID)
...
write.xlsx(Mergedd_doc_3, 'Mergedd_doc_3.xlsx')

a <- unique(Mergedd_doc_3$KID)
...
Duplicate_Furnace <- Furnace[duplicated(Furnace$OBJECTID),]

Unique_Furnace <- Furnace[!duplicated(Furnace$OBJECTID),]

Duplicate_HP <- HP[duplicated(HP$OBJECTID),]

write.xlsx(Duplicate_HP, 'Duplicate_HP.xlsx')


UNIQUE_HP <- HP[!duplicated(HP$OBJECTID),]

write.xlsx(UNIQUE_HP, 'UNIQUE_HP.xlsx')


write.xlsx(Unique_Furnace, 'UUnique_Furnace.xlsx')

hist(HP$`Keyword numbers`)


S_joint_data <- merge(new_joint, CEEMap_table_of_attributes, by.x = 'KID', by.y = 'KID')


write.xlsx(S_joint_data, 'S_joint_data.xlsx')
...
R_CEEMAp <- merge(BuildingOutlineWID_M6_Add, CEEMap_table_of_attributes, by.x = 'KID', by.y = 'KID')

R_CEEMAp_Permit <- merge(R_CEEMAp, KelownaPermitData, by.x = 'KID', by.y = 'KID')

library(xlsx)

write.csv(R_CEEMAp_Permit, 'R_CEEMAp_Permit.csv')
...
#Stack <- stack[substr(stack$WORK_DESCR,1,1)=="furnace","newCol"]<-"Furnace"
...
#myby <- by(stack$Building_PK,stack$FURNACE, function(x)paste(x,collapse=","))
...
#JU$initType <- grepl("furnace", JU$WORK_DESCR)
